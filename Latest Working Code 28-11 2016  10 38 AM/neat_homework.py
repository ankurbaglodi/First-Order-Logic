...
#==========================================================================#
def removeImplyOrig(x):
    if len(x) == 3 and x[1] == "=>" and type(x) == list:
        x[1] = "|"
        listing1 = []
        listing1.append("~")
        listing1.append(removeImplyOrig(x[0]))
        listing2 = removeImplyOrig(x[2])
        x[0] = listing1
        x[2] = listing2
        return x
    else:
...
def removeImply(x):
    if len(x) == 3 and x[1] == "=>" and type(x) == list:
        ...
        x[2] = listing2
        return x
    elif  len(x) == 2 and type(x) == list and type(x[1]) == list and x[1][1] == '=>' :
        temp = x[1]
        temp[1] = '|'
        listing1 = []
        listing1.append("~")
        listing1.append(removeImply(temp[0]))
        listing2 = removeImply(temp[2])
        temp[0] = listing1
        temp[2] = listing2
        x[1] = temp
        return x
...
def removeDistributionChild(x):
    ...
    if "&" == x[0][1] and "&" == x[2][1] and list == type(x[0]) and list == type(x[2]):
        #Now remove the disjunction and make it as conjuntion
        output.append((removeDistributionChild([x[0][0], "|" , x[2][0]])))
        output.append("&")
        output.append((removeDistributionChild([x[0][0], "|", x[2][2]])))
        output.append("&")
        output.append((removeDistributionChild([x[0][2], "|", x[2][0]])))
        output.append("&")
        output.append((removeDistributionChild([x[0][2], "|", x[2][2]])))
    else:
        #Handle second case
        if "&" == x[0][1] and list == type(x[0]):
            ...
            if len(x[2]) == 3 :
                ...
                if retCode:
                    ...
                    #It will be in the same form as before
                    output.append((removeDistributionChild([x[0][0] , "|" , x[2] ])))
                    output.append("&")
                    output.append((removeDistributionChild([x[0][2], "|", x[2]])))
            else:
                # It will be in the same form as before
                output.append((removeDistributionChild([x[0][0], "|", x[2]])))
                output.append("&")
        ...
        #Handle third case
                ...
                if len(x[0]) == 3:
                    ...
                    if retCode:
                        ...
                        output.append((removeDistributionChild([x[0] , "|" , x[2][0]])))
                        output.append("&")
                        output.append((removeDistributionChild([x[0], "|" , x[2][2]])))
                else:
                    output.append((removeDistributionChild([x[0], "|", x[2][0]])))
                    output.append("&")
                    output.append((removeDistributionChild([x[0], "|", x[2][2]])))
...
def reduceTheKB(x):
    if list == type(x) and x[1] != '&':
        #Check (a | (b&c) )
        if 3 == len(x) and list == type(x) and (str == type(x[0]) or (list == type(x[0]) and x[0][0] == '~')) and list == type(x[2]):
            right = x[2]
            if 3 == len(right) and list == type(right) and (str == type(right[0]) or (list == right[0] and right[0][0] == '~')) and\
                    (str == type(right[2]) or (list ==  type(right[2]) and right[2][0] == '~')):
        ...
        elif 3 == len(x) and list == type(x) and list == type(x[0]) and (str == type(x[2]) or (list == type(x[2]) and x[2][0] == '~')):
            left = x[0]
            if 3 == len(left) and list == type( left) and (str == type(left[0]) or (list == left[0] and left[0][0] == '~')) and\
                    (str == type(left[2])  or (list ==  type(left[2]) and left[2][0] == '~')):
                return removeDistributionChild(x)
            else:
                x[0] = reduceTheKB(x[0])
                x[2] = reduceTheKB(x[2])
        ...
        if x[1] == "&" and len(x) > 3:
            ...
            while (len(queue) != 0):
                ...
                if list == type(temp) and temp[1] == '|':
                    ...
                    output.append(temp)
                elif list == type(temp) and temp[0] == "~" and len(temp) == 2 and type(x[1]) == str:
                    output.append(temp)
                else:
...
# def removeInnerBracket(x):
#     if x[1] == "&" and len(x) >= 3 and type(x) == list:
#         output = []
#         queue = []
#         for iterator in range(len(x)):
#             if list == type(x) and x[iterator] != '&' :
#                 queue.append(x[iterator])
#         while (len(queue) != 0):
#             temp = queue.pop(0)
#             if list == type(temp) and temp[1] == '|':
#                 # and (type(temp[0]) == str or type(temp[0]) == list and len(temp[0]) == 2 and temp[0][0] == '~' and type(temp[0][1]) == str)\
#                 #     and \
#                 #     (type(temp[1]) == str or type(temp[1]) == list and len(temp[1]) == 2 and temp[1][0] == '~' and type(temp[1][1]) == str):
#                 output.append(temp)
#             elif str == type(temp):
#                 output.append(temp)
#             elif list == type(temp) and temp[0] == "~" and len(temp) == 2 and type(x[1]) == str:
...
def replaceVariableInAPredicate(x,dictionary):
    if str == type(x) or (list == type(x) and 2 == len(x)):
        tempReplace = replaceVariableInAPredicateChild(x, dictionary)
        if checkIfAtLeastOneLitrel(tempReplace) == True:
            return  tempReplace
        return None
        ...
        for iterator in x:
            if iterator != '~' and iterator != '&' and iterator != '|':
                tempReplace = replaceVariableInAPredicateChild(iterator, dictionary)
                newPred.append(tempReplace)
                newPred.append("|")

        if checkIfAtLeastOneLitrel(newPred[:-1]) == True:
            return newPred[:-1]
        return None

def checkIfAtLeastOneLitrel(x):
    if type(x) == str or (list == type(x) and 2 == len(x) and x[0] == '~'):
        atom = splitThePredicate(x)
        for iterator in atom[1]:
            if iterator[0].isupper() == True:
                return True
        return False
    elif type(x) == list and len(x) >= 3:
        for iterator in x:
            if iterator != '|' and iterator != '&' and iterator != '~':
                atom = splitThePredicate(iterator)
                for subiterator in atom[1]:
                    if subiterator[0].isupper() == True:
                        return True
        return False

def checkIfAtLeastOneVariable(x):
    if type(x) == str or (list == type(x) and 2 == len(x) and x[0] == '~'):
        atom = splitThePredicate(x)
        for iterator in atom[1]:
            if iterator[0].islower() == True:
                return True
        return False
    elif type(x) == list and len(x) >= 3:
        for iterator in x:
            if iterator != '|' and iterator != '&' and iterator != '~':
                atom = splitThePredicate(iterator)
                for subiterator in atom[1]:
                    if subiterator[0].islower() == True:
                        return True
        return False

def replaceVariableInAPredicateChild_orig(x,dictinonary):
    newVariables = []
    isNegated = False
    ...
    for variable in splitPredicateForCreatingNewOne[1]:
        if variable != '~' and variable!= '&' and variable != '|':
            if variable[0].isupper() == True:
                ...
                if variable in dictinonary:
                    # get the value
                    value = dictinonary[variable]
                    while value in dictinonary:
                        key = value
                        value = dictinonary[key]
                    newVariables.append(value)
                elif variable in dictinonary.values():
                    # get the value
                    value = variable
                    while value in dictinonary:
            ...
            for tempSubIterator in secondToCheck:
                if (list == type(tempIterator) and 2 == len(tempIterator) and str == type(tempSubIterator)) and \
                    ...
                    if tempOfFirst[0] == tempOfSecond[0]:
                        ...
                        copyOfFirst     = replaceVariableInAPredicate(firstToCheck,dictionary)
                        copyOfSecond    = replaceVariableInAPredicate(secondToCheck,dictionary)
                        if copyOfFirst == None or copyOfSecond == None:
                            return None
                        newTempPredicateToWorkOn = []
...
def resolutionBFS(KBList,statement,tableIndexPositive,tableIndexNegative):
    ...
    while True:
        ...
        for iterator in KBListToActOn:
            ...
            for superAnswer in tempList:
                    for answerIterator in superAnswer:
                        ...
                        if isPresentInKB == False and len(answerIterator) > 0 and checkIfAtLeastOneLitrel(answerIterator) == True:

                            if isFact == True:
                                if answerIterator not in newList:
                                    newList.insert(0,answerIterator)
                                    # print "Answer " + str(answerIterator)
                                    superCounter += 1
                            else:

                                #     hasVariable = False
                                #     for variableIterator in answerIterator:
                                #         if variableIterator != "|" and variableIterator != "&" and variableIterator != "~":
                                #             hasVariable = checkIfAtLeastOneVariable(variableIterator)
                                #             if hasVariable == True:
                                #                 break
                                #     if hasVariable == True:
                                if answerIterator not in newList:
        ...
        if superCounter == 0:
            return False
        newList.extend(KBListToActOn)
        KBListToActOn = copy.copy(newList)
...
if __name__ == "__main__":
    ...
    tempDict = {}
    tempDict = newUnify(["x1","x2"],["x3","x4"],tempDict)
    print tempDict
    r = []
    # Call resolution
    ...
    # print resolveFurtherBruteForceNextVersion("N(Brick)", [['~', 'N(x1)'], '|', 'A(Nick)'])